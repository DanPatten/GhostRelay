...
Check the user's message arguments (everything after `/ghostrelay`). If the argument is or starts with `endless`, follow the **Endless polling mode** workflow below. Otherwise, follow the **One-shot mode** workflow.
...
When the user runs `/ghostrelay endless`, enter a continuous monitoring loop that watches for tagged elements and processes them automatically.
